...
import { boxApply, boxNameSymbol, boxNativeFunc, boxNil, boxRoundBlock, Thing, ThingType, typecheck } from "../objects/thing";
...
export function initCoreSyntax(env: Thing<ThingType.env>, functions: Record<string, NativeFunctionDetails>) {
    ...
    define_pattern(env, functions, "[^]x y...[$]", STANDARD_BLOCKS, "__builtin_apply", (task, state) => {
        const groups: Thing<ThingType.map> = state.argv[0]! as any;
